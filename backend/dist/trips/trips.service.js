...
let TripsService = class TripsService {
    ...
    async getCaptainTrips(userId) {
      ...
      FROM trip_data td;
    `;
        const result = await this.prisma.$queryRawUnsafe(rawSql);
        return result;
    }
    async getDepotTrips(userId) {
        const stations = await this.prisma.$queryRawUnsafe(`
    ...
    async getDepotCaptainsOverall(userId) {
        const stations = await this.prisma.$queryRawUnsafe(`
      SELECT "A" FROM "public"."_StationsToUser" WHERE "B" = ${userId}
    `);
        if (!stations || stations.length === 0)
    ...
    `;
        return await this.prisma.$queryRawUnsafe(rawSql);
    }
    ...
    `);
        ...
        const stationIds = stations.map(s => s.A).join(',');
        const allCaptainsRaw = await this.prisma.$queryRawUnsafe(`
      SELECT "id", CONCAT("firstName", ' ', COALESCE("lastName", '')) AS name
      FROM "public"."User"
      WHERE "depotStationId" IN (${stationIds}) AND "active" = true AND "userTypeId" = 4
      ORDER BY "firstName" ASC
    `);
        const assignedYesterdayRaw = await this.prisma.$queryRawUnsafe(`
    ...
    `);
        const assignedIds = new Set(assignedYesterdayRaw.map((r) => r.captainId));
        const unassigned = allCaptainsRaw.filter((c) => !assignedIds.has(c.id));
        return {
            totalCaptains: allCaptainsRaw.length,
            assignedYesterday: assignedYesterdayRaw.length,
            unassignedYesterday: unassigned.length,
            unassignedCaptains: unassigned.map((c) => ({
                id: c.id,
                name: c.name.trim(),
            })),
        };
    ...
    async getOpsDashboardMetrics() {
        const globalMetricsRaw = await this.prisma.$queryRawUnsafe(`
      WITH today_trips AS (
        SELECT DISTINCT
          NULLIF(SPLIT_PART(tq."answer", '-', 1), '')::INTEGER AS "captainId"
        FROM "public"."TripQuestions" tq
    ...
    `);
        const depotPerformanceRaw = await this.prisma.$queryRawUnsafe(`
      WITH depot_data AS (
        SELECT 
          st."id" AS "stationId",
      ...
      FROM depot_data
    `);
        const trendDataRaw = await this.prisma.$queryRawUnsafe(`
      WITH date_range AS (
    ...
    `);
        ...
        return {
            ...
            depotPerformance: depotPerformanceRaw.map((depot) => ({
                name: depot.name,
            ...
            trendData: trendDataRaw.map((trend) => ({
                date: trend.date,
